...
describe('ChatDisplay — cluster enrollment', () => {
  it('shows Identify button for final entries with cluster_label', () => {
    ...
    render(
      <ChatDisplay
        ...
        contacts={NO_CONTACTS}
        showCallsignChips={false}
        onEnrollCluster={onEnroll}
      />
    )
    expect(screen.getByRole('button', { name: /Identify SPK_A/i })).toBeInTheDocument()
  })

  it('does not show Identify button when entry is partial', () => {
    const onEnroll = vi.fn()
    render(
      <ChatDisplay
        entries={[
  ...
  it('does not show Identify button when onEnrollCluster is not provided', () => {
    render(
      <ChatDisplay
        entries={[makeEntry({ cluster_label: 'SPK_A', partial: false })]}
        contacts={NO_CONTACTS}
        showCallsignChips={false}
      />
    )
  ...
  it('calls onEnrollCluster with cluster label and trimmed uppercase callsign', () => {
    const onEnroll = vi.fn()
    const promptSpy = vi.spyOn(window, 'prompt').mockReturnValue('  kd9xyz  ')
    render(
      <ChatDisplay
        entries={[
          makeEntry({
    ...
    fireEvent.click(screen.getByRole('button', { name: /Identify SPK_A/i }))
    expect(onEnroll).toHaveBeenCalledWith('SPK_A', 'KD9XYZ')
    promptSpy.mockRestore()
  })

  it('does not call onEnrollCluster when prompt is cancelled', () => {
    const onEnroll = vi.fn()
  ...
  it('does not call onEnrollCluster when prompt returns empty string', () => {
    const onEnroll = vi.fn()
